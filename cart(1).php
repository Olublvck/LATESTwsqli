...
if(isset($_POST['order_btn'])){
  ...
  $mobnumber = htmlspecialchars($_POST['mobnumber']);
...
$id=htmlspecialchars($_SESSION['userid']);
 ...
 if(isset($_POST['update_update_btn'])){
  $update_value = htmlspecialchars($_POST['update_quantity']);
  $update_id = htmlspecialchars($_POST['update_quantity_id']);
  ...
  <?php
   $total=0;
    ...
    <tr>
      ...
      <td><form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8'); ?>" method="post">
      <input type="hidden" name="update_quantity_id"  value="<?php echo htmlspecialchars($row['id'], ENT_QUOTES, 'UTF-8'); ?>" >
      <input type="number" name="update_quantity" min="1"  value="<?php echo htmlspecialchars($row['quantity'], ENT_QUOTES, 'UTF-8'); ?>" >
        <input type="submit" value="update" name="update_update_btn">
      </form></td> 
      <td><?php echo htmlspecialchars($row["price"]*$row["quantity"], ENT_QUOTES, 'UTF-8') ?></td>
      <?php $total=$total+$row["price"]*$row["quantity"] ;?>
     

      <input type="hidden" name="status" value="pending">   
      <td><a href="cart(1).php?remove=<?php echo htmlspecialchars($row['id'], ENT_QUOTES, 'UTF-8'); ?>">remove</a></td>
    </tr>
    <?php 
    }
...
<div class="myDiv">
<?php echo "<h style='font-size: 24px;'>Total Amount= ".htmlspecialchars($total, ENT_QUOTES, 'UTF-8') . "</h>"; ?>
</div>
